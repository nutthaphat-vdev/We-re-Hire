...
## Day 5 — 27 พฤษภาคม 2568 · วัน Stability & GTM
...
**XSS ที่แก้ครอบคลุม:** job title, location_name, skills array, employer_note, full_name, notification title/body, contact name/phone/email, employer company_name

**คงเหลือ (ก่อน scale):**
W1 rate limiting · C2 JWT expire 120 นาที · W7 `is_active` ใน `get_current_user` · I1 security headers · W5 column allowlist

### · 📄 ใบปลิว GTM — Dark + Light Flyer
สร้างใบปลิว 2 แบบสำหรับแจกหน้าโรงงาน นิคมอุตสาหกรรม และโพสต์ออนไลน์:

| ไฟล์ | Theme | ใช้สำหรับ |
|------|-------|---------|
| `WeHired_DarkFlyer.html` | Dark mode | โพสต์ Facebook / LINE / Online |
| `WeHired_LightFlyer.html` | Light mode | พิมพ์แจก — A4 → ตัด 2x A5 |

- QR Code ลิงก์ไป frontend + แสดง URL
- Phase 3 teaser: "เร็วๆ นี้: ระบบ Wallet + รับเงินผ่าน PromptPay"

### · 🔗 Infrastructure — GitHub Repo + Railway Reconnect
- ย้าย repo ไปอยู่ใน account ใหม่ `nutthaphat-vdev/We-re-Hire`
- Revoke PAT เก่า (`abc147258`) + สร้าง PAT ใหม่ scope: `repo` + `workflow`
- Railway → Settings → Source → reconnect repo ใหม่
- ยืนยัน 100%:
  ```json
...
### · 🐛 Bug #7 — Railway Source Repo Webhook (~2 ชม.)
```
...
GitHub redirect repo อัตโนมัติ → ดูเหมือนทำงาน แต่ Railway ไม่ได้รับ push webhook จริง
→ Railway deploy ไปที่ service ใหม่ `web-production-1db39` แทน (คนละ service!)

**Debug process — พี่เป็นคนเจอ root cause:**
1. เพิ่ม `build` field ใน `/health` → response ไม่มี field ใหม่ = code เก่า
2. curl 5 รอบ → URL เก่าทุกรอบ = ไม่ใช่ load balance
3. พบว่า `web-production-1db39.up.railway.app` คือ service ใหม่ที่ deploy ถูก
...
- `CLAUDE.md`: เพิ่มข้อ 8 "Railway Source Repo" ใน Production URL Change Checklist

**Lesson:** GitHub redirect เงียบๆ แต่ Railway webhook ยังผูกกับ repo เก่า — ต้อง reconnect มือทุกครั้ง
